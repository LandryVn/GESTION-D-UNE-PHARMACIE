...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Parcourir tous les médicaments achetés
        for ($i = 0; $i < count($numMedocs); $i++) {
            ...
            // Calcul du montant pour ce médicament
            $montant = $nbr * $medoc['prix_unitaire'];
            $recetteAchat += $montant;

            // Ajouter le détail de l'achat
            ...
            // Sauvegarde du Design dans DETAIL_ACHAT
        ...
        if ($recetteAchat > 0) {
            // 1. Vérifier si la table stats_recette_totale existe et contient une ligne
            $stmt = $pdo->query("SELECT COUNT(*) FROM stats_recette_totale");
            if ($stmt->fetchColumn() == 0) {
                // Si la table est vide, insérer une première ligne
            ...
            // 2. Mise à jour stats mensuelles
            $stmt = $pdo->prepare("
